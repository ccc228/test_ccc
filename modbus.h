...
typedef struct _modbus_mapping_t {
    int nb_bits;                    //有多少个(数量） 从起始点算
    int start_bits;                     //线圈数量 起始点
    int nb_input_bits;              //有多少个(数量） 从起始点算
    int start_input_bits;               //离散量输入(开关量) 起始点
    int nb_input_registers;
    int start_input_registers;          //输入寄存器 起始点
    int nb_registers;               //有多少个(数量）
    int start_registers;                //保持寄存器
    uint8_t *tab_bits;              //线圈存储区指针
    uint8_t *tab_input_bits;        //多开关量存储区指针
    uint16_t *tab_input_registers;  //多输入寄存器存储区指针
    uint16_t *tab_registers;        //多保持寄存器存储区指针
} modbus_mapping_t; //结构体变量
